...
class TestRunQDEvolution:
    ...
    def test_run_qd_evolution_returns_portfolio_and_coverage(self) -> None:
        ...
        filled_bins = cast(int, result.coverage_report["filled_bins"])
        assert filled_bins > 0
...
def test_run_qd_evolution_falls_back_to_pareto_front_when_archive_empty() -> None:
    ...
    try:
        ...
        result = run_qd_evolution(
            ...
            evaluator=_ToyEvaluator(),
            context=context,
            archive=cast(Any, _ArchiveStub(n_dims=1, elites=[])),
            behavior_descriptor_names=(BEHAVIOR_DESCRIPTOR_TURNOVER,),
            portfolio_size=3,
        )
...
def test_seed_archive_skips_invalid_fitness_and_honors_insert_errors() -> None:
    ...
    invalid = FitnessResult(objectives=cast(Any, ("bad",)), metadata={})
...
def test_qd_parent_source_applies_coverage_interval_and_minimum_pick() -> None:
    ...
    def _select(
        ...
        *,
        ...
        ranks: list[int] | None,
        crowding: list[int | float] | None,
    ) -> list[Any]:
        del population, fitnesses, config, rng, fronts, ranks, crowding
        return [
            999,
            998,
        ]

    qd.orchestrator.select = cast(Any, _select)
    archive = _Archive()
    try:
        ...
        first = source([1], [fitness], config, rng, 2, ctx)
        second = source([2], [fitness], config, rng, 2, ctx)
        assert first[:1] == [111]
        assert len(second) == 2
    finally:
